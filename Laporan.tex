...
\begin{@twocolumnfalse}
  ...
  \textit{systemd} dengan \textit{scrape interval} 5 detik dan \textit{evaluation interval} 5 detik. Pengujian dilakukan melalui simulasi
...
sehingga sesuai dengan kendala anggaran pemerintah daerah \cite{brikman2022}. Penelitian ini merancang
...
dapat diakses oleh \textit{server} Prometheus \cite{brikman2022}.
...
\cite{brikman2022,cncf2023}.

\subsection{Grafana}

Grafana adalah platform visualisasi data \textit{open-source} yang mendukung Prometheus sebagai
\textit{data source} secara \textit{native} \cite{brendan2020}. Grafana menyediakan berbagai jenis
...
untuk mengukur \textit{throughput} \cite{Rahayu2023}. \texttt{stress-ng} adalah utilitas pengujian
...
et al.\ \cite{Rahayu2023} mengembangkan sistem \textit{monitoring real-time} berbasis Zabbix dengan
...
+ Grafana + Alertmanager) telah terbukti andal pada lingkungan serupa \cite{Rahman2020,Rahayu2023},
...
industri \cite{turnbull2018,brikman2022}. Nilai CPU dan memori sebesar 80\% ditetapkan untuk
...
\textit{scrape interval} 5 detik, \textit{scrape timeout} 4 detik, dan \textit{evaluation interval} 5 detik. Verifikasi aliran data metrik
...
\cite{turnbull2018,brikman2022}. Seluruh aturan menggunakan klausul \texttt{for: 1m} sehingga
...
%   rate_window (60s) + for:1m (60s, overlap) + group_wait (10s) + pengiriman (~5s) = ~75s
%   evaluation_interval 5s sehingga overhead tunggu evaluasi minimal (<5s)
Rata-rata \textit{alert latency} yang dicapai adalah $[\mathit{X}]$ detik
($\text{SD} = [\mathit{Y}]$ detik). Berdasarkan konfigurasi sistem (\texttt{scrape\_interval} 5
detik, \texttt{evaluation\_interval} 5 detik, klausul \texttt{for} 1 menit,
\texttt{group\_wait} 10 detik), latensi teoritis minimum adalah 70--80 detik sejak ambang batas
pertama kali terlampaui hingga notifikasi diterima. Dengan \texttt{evaluation\_interval} 5 detik,
overhead menunggu siklus evaluasi berikutnya tidak melebihi 5 detik, sehingga latensi lebih
konsisten dibandingkan konfigurasi \texttt{evaluation\_interval} bawaan 1 menit. Hasil per skenario
...
\bibitem{brendan2020}
  Brendan G. 2020.
  \textit{Grafana: Infrastructure Monitoring}.
  Birmingham (UK): Packt Publishing.

\bibitem{bssn2021}
  BSSN. 2021.
  \textit{Pedoman Teknis Disaster Recovery Center untuk Instansi Pemerintah}.
  Jakarta (ID): Badan Siber dan Sandi Negara.

\bibitem{cncf2023}
  Cloud Native Computing Foundation. 2023.
  Prometheus Documentation [Internet]. [diunduh 2025 Jan 10].
  Tersedia pada: \url{https://prometheus.io/docs}.

\bibitem{grafana2024}
  Grafana Labs. 2024.
  Grafana Documentation v12.x [Internet]. [diunduh 2026 Jan 10].
...
\bibitem{linux2023}
  ...
  Systemd Documentation [Internet]. [diunduh 2025 Jan 10].
  Tersedia pada: \url{https://systemd.io}.

\bibitem{telegram2023}
  Telegram. 2023.
  Telegram Bot API Documentation [Internet]. [diunduh 2025 Jan 15].
...
\bibitem{Rahman2020}
  Rahman M, Budiyanto H, Prasetya B. 2020.
  Monitoring \textit{server} dengan Prometheus dan Grafana serta notifikasi Telegram.
  \textit{JITSI: Jurnal Ilmiah Teknologi Sistem Informasi}. 10(2):112--120.
  doi: 10.62527/jitsi.1.4.19

\bibitem{Rahayu2023}
  Rahayu S, Hidayat A, Kurniawan D. 2023.
  \textit{Monitoring real-time server} dan \textit{router} berbasis Zabbix dengan notifikasi
  \textit{alert} Telegram.
  \textit{Intellect: Jurnal Ilmiah Komputer dan Informasi}. 8(1):45--55.
